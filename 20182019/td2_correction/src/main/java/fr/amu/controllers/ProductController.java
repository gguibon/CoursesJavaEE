...
//    }
	...
	@RequestMapping(value = "/add", method = RequestMethod.POST)
    public String add(HttpServletRequest request, Map<String, Object> model){ //nous avons bien la requete et ce qu'on veut donner dans la réponse
		// les paramètres viennent d'un formulaire sous POST présent dans la vue (jsp). Par exemple : name = "category" , value = "computer"
		...
		ps.addProduct(p);
		// on ajoute l'attribut "products" dans la réponse avec pour valeur la List<Product> de ps.getProducts(); 
		// dans la vue nous y accéderons avec l'Expression Language (EL) de cette manière : ${ products }
		model.put("products", ps.getProducts() ); 
    	return "homepage";
    }
